...
| `recorded_time` | `[rec:YYYY-MM-DD]` prefix; for v1 entries without it, the file modification or commit timestamp of the line | entry author (normalizer fallback) |
...
| `supersedes` / `superseded_by` | resolved from the date in the mark, which equals the replacement entry's `rec`; content similarity for undated v1 marks (best effort) | normalizer |
