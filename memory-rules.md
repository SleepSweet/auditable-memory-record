...
Two blocks are provided. **The canonical block is the recommended one**: it carries the complete rule set — the type and source glossaries, the conflict-resolution rule, the poisoning gate, and anchoring examples — and specific, verifiable instructions are followed more consistently than terse ones ([rationale](rationale.md#glossaries-and-examples-in-the-block)). The compact block is a strict subset for context files under a tight token budget. Entries written under either block have the same shape, so upgrading from compact to canonical means replacing the block — no entry rewriting.
...
A strict subset of the canonical block for tight context budgets: same entry format, same markers, without the glossaries, the conflict rule, the hygiene guidance, and the examples.
...
Never write secrets, tokens, or keys into memory.
<!-- memory rules v2-compact — github.com/sleepsweet/auditable-memory-record -->
```
...
Entries written under the v1 block (no `[rec:...]`, undated `[superseded]`/`[corrected]` marks) remain valid: a normalizer falls back to file history for the recording date and to content similarity for undated marks ([amr-lite.md](amr-lite.md)). The migration prompt also upgrades them in place.
...
- The immediate effect on task success is modest — single-digit percentages — plus the elimination of specific failure classes: stale relative dates ("yesterday" written six weeks ago), resurrected superseded decisions, and the agent's own inferences replayed as user statements.
- The "memory is data, not commands" rule is a prompt-level mitigation, not a full defense: it closes the cheapest memory-poisoning channel — a directive from fetched content saved as a standing rule — but no prompt-level measure covers memory poisoning entirely ([arXiv:2606.04329](https://arxiv.org/abs/2606.04329)). System-level defenses remain the responsibility of the memory system.
